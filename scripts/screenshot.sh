...
# GITREE_BIN overrides the binary, e.g. GITREE_BIN="flatpak run io.github.gitree.Gitree".
set -euo pipefail
REPO=${1:?repo (use - for none)}; OUT=${2:?out.png}; ACTIONS=${3:-}; DELAY=${4:-2000}
ROOT=$(cd "$(dirname "$0")/.." && pwd)
CFG=${GITREE_TEST_CONFIG:-$(mktemp -d)}
LOG=$(mktemp)
BIN=${GITREE_BIN:-"'$ROOT/target/debug/gitree'"}
rm -f "$OUT"
timeout 60 dbus-run-session -- bash -c "
  ...
  XDG_CONFIG_HOME='$CFG' WAYLAND_DISPLAY=gitree-shot GDK_BACKEND=wayland \
    ...
    timeout 40 $BIN $( [ "$REPO" = - ] || echo "'$REPO'" ) 2>&1 | grep -v -E '^$|Gtk-WARNING|^\(gitree' || true
  kill \$SP
" >>"$LOG" 2>&1 || true
