...
pub struct DiffView {
    ...
    /// Hunk views' horizontal adjustments, scrolled together by `hoffset`.
    hadjs: RefCell<Vec<gtk::Adjustment>>,
    hoffset: Cell<f64>,
    syncing: Cell<bool>,
...
impl DiffView {
    pub fn new() -> Rc<Self> {
        ...
        let this = Rc::new(Self {
            ...
            hadjs: RefCell::new(Vec::new()),
            hoffset: Cell::new(0.0),
        ...
        *this.self_ref.borrow_mut() = Rc::downgrade(&this);
        ...
        // Sideways scrolling (touchpad or Shift+wheel) moves all hunks together,
        // leaving the gutters and hunk headers in place. Vertical deltas go on
        // to the scrolled window.
        let scroll = gtk::EventControllerScroll::new(gtk::EventControllerScrollFlags::BOTH_AXES);
        scroll.set_propagation_phase(gtk::PropagationPhase::Capture);
        let w = Rc::downgrade(&this);
        scroll.connect_scroll(move |c, dx, dy| {
            let (Some(d), Some((dx, only))) = (w.upgrade(), super::horizontal_scroll(c, dx, dy)) else {
                return glib::Propagation::Proceed;
            };
            d.scroll_h(dx);
            if only { glib::Propagation::Stop } else { glib::Propagation::Proceed }
        });
    ...
    pub fn debug_select_lines(&self, spec: &str) {
        ...
        while let Some(c) = child {
            if let Some(body) = c.downcast_ref::<gtk::Box>()
                && let Some(view) = body.last_child().and_downcast::<gtk::ScrolledWindow>().and_then(|s| s.child()).and_then(|v| v.first_child()).and_downcast::<sourceview5::View>() {
                    if n == h {
                        let buf = view.buffer();
    ...
    fn scroll_h(&self, dx: f64) {
        let max = self
            ...
            .fold(0.0, f64::max);
        self.set_hoffset((self.hoffset.get() + dx).clamp(0.0, max));
    }

    fn set_hoffset(&self, v: f64) {
        self.hoffset.set(v);
        self.syncing.set(true);
        for a in self.hadjs.borrow().iter() {
            a.set_value(v);
        }
    ...
    fn render_hunk(&self, f: &FileDiff, hi: usize, h: &crate::git::diff::Hunk, lang: Option<&sourceview5::Language>) {
        ...
        // The view is sized to its longest line inside a viewport (rather than
        // scrolling itself) so line backgrounds span the whole width. Scroll
        // events are handled by `DiffView`, which keeps all hunks in step.
        let hview = gtk::ScrolledWindow::builder()
            .child(&gtk::Viewport::builder().child(&view).build())
            .vscrollbar_policy(gtk::PolicyType::Never)
            .propagate_natural_height(true)
            .hexpand(true)
            .build();
        let controllers = hview.observe_controllers();
        for c in (0..controllers.n_items()).filter_map(|i| controllers.item(i)) {
            if let Some(c) = c.downcast_ref::<gtk::EventControllerScroll>() {
                c.set_propagation_phase(gtk::PropagationPhase::None);
            }
        }
        let hadj = hview.hadjustment();
        let w = self.self_ref.borrow().clone();
        hadj.connect_changed(move |a| {
            if let Some(d) = w.upgrade() {
                d.syncing.set(true);
                a.set_value(d.hoffset.get());
                d.syncing.set(false);
            }
        });
        let w = self.self_ref.borrow().clone();
        hadj.connect_value_changed(move |a| {
            if let Some(d) = w.upgrade()
                && !d.syncing.get() {
                    d.set_hoffset(a.value());
                }
        });
        self.hadjs.borrow_mut().push(hadj);
        ...
        view.set_size_request(layout.pixel_size().0 + view.left_margin() + view.right_margin() + 16, -1);
